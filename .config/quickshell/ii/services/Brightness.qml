...
Singleton {
    ...
    onMonitorsChanged: {
        ddcMonitors = [];
        ddcCheckProc.running = true;
    }

    // Check if ddcutil exists first
    Process {
        id: ddcCheckProc
        command: ["which", "ddcutil"]
        onExited: (exitCode, exitStatus) => {
            if (exitCode === 0) {
                // ddcutil exists, use it
                ddcProc.running = true;
            } else {
                // ddcutil not found, skip directly to initialization
                console.log("[Brightness] ddcutil not found, using brightnessctl");
    ...
    function ddcDetectFinished(): void {
        console.log("[Brightness] ddcDetectFinished called, initializing monitors");
        initializeMonitor(0);
    }
    ...
    Process {
        ...
        onExited: (exitCode, exitStatus) => {
            console.log("[Brightness] ddcProc exited with code:", exitCode);
            root.ddcDetectFinished()
        }
    }
    ...
    component BrightnessMonitor: QtObject {
        ...
        readonly property Process initProc: Process {
            stdout: SplitParser {
                onRead: data => {
                    console.log("[Brightness] initProc data:", data);
                    const [, , , current, max] = data.split(" ");
                    monitor.rawMaxBrightness = parseInt(max);
                    monitor.brightness = parseInt(current) / monitor.rawMaxBrightness;
                    monitor.ready = true;
                    console.log("[Brightness] Monitor ready, brightness:", monitor.brightness);
                }
            }
            onExited: (exitCode, exitStatus) => {
                console.log("[Brightness] initProc exited:", exitCode);
